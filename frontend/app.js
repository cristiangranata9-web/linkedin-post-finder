...
// --- Ricerca Soci/Partner (Cluster MINIT): elenco .docx/.xlsx (aziende e/o email), generale o per tematica ---
...
function renderCompanyTables(data) {
  ...
  // Vista settimanale
  const wHead = `<thead><tr><th>Azienda/Socio o Email</th><th>Nome pagina LinkedIn</th><th>URL pagina LinkedIn</th>${GIORNI_IT.map((g) => `<th>${g}</th>`).join("")}</tr></thead>`;
  ...
  const dHead = `<thead><tr><th>Azienda/Socio o Email</th><th>Nome pagina LinkedIn</th><th>URL pagina LinkedIn</th><th>Data del post</th><th>Giorno della settimana</th><th>Link al post</th>${topicHeader}</tr></thead>`;
